...
GUI::GUI(Client* client) : client(client) { clear_color = ImVec4(0.45f, 0.55f, 0.60f, 1.00f); }
...
int GUI::init()
{
    ...
    window = glfwCreateWindow(1280, 720, "Piupiu", NULL, NULL);
...
int GUI::render()
{
    ...
    static char                usernameInput[128];
    static char                text[1024 * 16];
    static char                ipAddr[100];
    static int                 port  = 5000;
    static ImGuiInputTextFlags flags = ImGuiInputTextFlags_AllowTabInput;

    strcpy(usernameInput, "fernando");
    ...
    if (loggedIn)
    {
        ...
        ImGui::Begin("Piupiu");
        ImGui::Text("Logado como @%s", usernameInput);
        ImGui::Text("Feed de mensagens");

        ImGui::InputTextMultiline("##source", text, IM_ARRAYSIZE(text),
                                  ImVec2(-FLT_MIN - 100, ImGui::GetTextLineHeight() * 16), flags);
        ImGui::SameLine();

        if (ImGui::Button("Enviar", ImVec2(100, ImGui::GetTextLineHeight() * 16)))
        {
            client->Post(std::string(text));
            memset(text, 0, 1024 * 16);
        }
    ...
    else
    {
        ...
        ImGui::Text("Área de login");
        if (errorMessage != nullptr)
        { ImGui::TextColored(ImVec4(255.f, .0f, .0f, .0f), errorMessage); }

        ImGui::InputText("Nome de usuário", usernameInput, 128);

        ImGui::InputText("Endereço IP", ipAddr, 100);
        ImGui::SameLine();
        HelpMarker("Endereço do servidor");

        ImGui::InputInt("Porta", &port);
        ImGui::SameLine();
        HelpMarker("Porta do servidor");

        if (ImGui::Button("Entrar", ImVec2(300, 50)))
        {
            if (strlen(usernameInput) > 4 && port > 0 && strlen(ipAddr) > 4)
            {
                loggedIn = true;
                client->Connect(usernameInput, std::string(ipAddr), port);
            }
            else
            {
                strcpy(usernameInput, "Nome de usuário muito curto");
            }
        }
